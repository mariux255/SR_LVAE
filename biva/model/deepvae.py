...
class DeepVae(nn.Module):
    ...
    def generate(self, posteriors: Optional[List],image = None, **kwargs) -> Dict[str, torch.Tensor]:
        """
        ...
        # output convolution
        #print("x shape = ", x['d'].shape) (5,96,32,32)
    ...
    def forward(self, x: torch.Tensor, **kwargs: Any) -> Dict[str, torch.Tensor]:
        ...
        image = x
        image = torch.from_numpy(resize(image.cpu(), output_shape = (x.shape[0],3,16,16), mode = 'reflect', anti_aliasing=True))
        ...
        data = self.generate(posteriors, image = image,N=x.size(0), **kwargs)

        return data

    def sample_from_prior(self, N: int, img = None, **kwargs: Any) -> Dict[str, torch.Tensor]:
        """
        ...
        """
        return self.generate(None, image = img, N=N, **kwargs)
